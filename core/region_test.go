...
// TestNTSCTiming verifies NTSC timing constants
func TestNTSCTiming(t *testing.T) {
...
// TestPALTiming verifies PAL timing constants
...
// TestGetVideoTiming verifies correct timing is returned per video standard
func TestGetVideoTiming(t *testing.T) {
	ntsc := GetVideoTiming(VideoNTSC)
	if ntsc.CPUClockHz != NTSCTiming.CPUClockHz {
		t.Errorf("GetVideoTiming(NTSC): expected NTSC timing, got CPUClockHz=%d", ntsc.CPUClockHz)
	}

	pal := GetVideoTiming(VideoPAL)
	if pal.CPUClockHz != PALTiming.CPUClockHz {
		t.Errorf("GetVideoTiming(PAL): expected PAL timing, got CPUClockHz=%d", pal.CPUClockHz)
	}
}

// TestScanlineTimingConsistency verifies timing relationships
func TestScanlineTimingConsistency(t *testing.T) {
