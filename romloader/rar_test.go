...
func TestExtractFromRAR_FileNotFound(t *testing.T) {
	_, _, err := extractFromRAR("/nonexistent/path/test.rar")
	if err == nil {
...
// TestLoadROM_RARFormatDetection tests that RAR files are detected correctly
func TestLoadROM_RARFormatDetection(t *testing.T) {
	// Test magic byte detection
	header := magicRAR
	format := detectFormat(header, "file.dat")
	if format != formatRAR {
		t.Errorf("RAR magic should be detected, got format %d", format)
	}

	// Test extension fallback
	format = detectFormat([]byte{}, "file.rar")
	if format != formatRAR {
...
// TestLoadROM_RAR_Integration tests LoadROM with RAR (expects failure without valid archive)
func TestLoadROM_RAR_Integration(t *testing.T) {
	tmpDir := t.TempDir()
...
// TestExtractFromRAR_DirectoryInArchive tests handling of directories in RAR
...
func TestExtractFromRAR_DirectorySkipping(t *testing.T) {
	...
	// but we can verify the detection logic handles the case where no .sms file is found
	tmpDir := t.TempDir()
	path := filepath.Join(tmpDir, "empty.rar")
	...
	_, _, err = extractFromRAR(path)
	// Should fail (can't read header or no SMS file)
	if err == nil {
