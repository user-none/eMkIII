...
func TestExtractFrom7z_FileNotFound(t *testing.T) {
	_, _, err := extractFrom7z("/nonexistent/path/test.7z")
	if err == nil {
...
// TestLoadROM_7zFormatDetection tests that 7z files are detected correctly
func TestLoadROM_7zFormatDetection(t *testing.T) {
	...
	format = detectFormat([]byte{}, "file.7z")
	if format != format7z {
		t.Errorf(".7z extension should be detected, got format %d", format)
	}
}

// TestLoadROM_7z_Integration tests LoadROM with 7z (expects failure without valid archive)
func TestLoadROM_7z_Integration(t *testing.T) {
	tmpDir := t.TempDir()
