...
type SharedInput struct {
	...
	smsPause                          bool // Edge-triggered: set by Ebiten, cleared on read
}

// Set updates directional and button state from the Ebiten thread.
func (si *SharedInput) Set(up, down, left, right, btn1, btn2 bool) {
	si.mu.Lock()
	si.up = up
	si.down = down
	si.left = left
...
// SetPause signals a pause button press (edge-triggered).
func (si *SharedInput) SetPause() {
	si.mu.Lock()
	si.smsPause = true
	si.mu.Unlock()
}

// Read returns the current input state and clears the pause flag.
func (si *SharedInput) Read() (up, down, left, right, btn1, btn2, smsPause bool) {
	si.mu.Lock()
	up = si.up
	down = si.down
	left = si.left
...
func (sf *SharedFramebuffer) Update(pixels []byte, stride, activeHeight int, leftColumnBlank bool) {
	sf.mu.Lock()
	...
	sf.leftColumnBlank = leftColumnBlank
	sf.mu.Unlock()
}
...
func (sf *SharedFramebuffer) Read() (pixels []byte, stride, activeHeight int, leftColumnBlank bool) {
	sf.mu.Lock()
	stride = sf.stride
	activeHeight = sf.activeHeight
	leftColumnBlank = sf.leftColumnBlank
	n := stride * activeHeight
	if n > len(sf.writePixels) {
		n = len(sf.writePixels)
	}
