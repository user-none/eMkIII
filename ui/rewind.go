...
func (rb *RewindBuffer) Capture(emulator *emubridge.Emulator) error {
	rb.frameTick++
...
func (rb *RewindBuffer) Rewind(emulator *emubridge.Emulator, count int) bool {
	if rb.count == 0 {
		return false
	}
	...
	// RunFrame to regenerate the framebuffer from VDP state
	emulator.RunFrame()
